...
function SessionsSection({ data, search, onChanged }: { data: AdminCurriculum; search: string; onChanged: () => void }) {
  ...
  const [showCreate, setShowCreate] = useState(false);
  const [message, setMessage] = useState("");
  const [draft, setDraft] = useState<AdminSessionInput>({ courseId: data.programs[0]?.id ?? "", dateTime: new Date().toISOString(), timezone: "America/New_York", subject: data.programs[0]?.subject ?? "", title: "", durationMinutes: 60, status: "draft", bookingStatus: "confirmed", clientUserId: data.clients[0]?.id ?? null, tutorUserId: data.tutors[0]?.id ?? null });
  const term = search.trim().toLowerCase();
  ...
  const reconnectMailto = selectedTutor
    ...
    : "";
  const reset = () => setDraft({ courseId: data.programs[0]?.id ?? "", dateTime: new Date().toISOString(), timezone: "America/New_York", subject: data.programs[0]?.subject ?? "", title: "", durationMinutes: 60, status: "draft", bookingStatus: "confirmed", clientUserId: data.clients[0]?.id ?? null, tutorUserId: data.tutors[0]?.id ?? null });
  const save = () => {
