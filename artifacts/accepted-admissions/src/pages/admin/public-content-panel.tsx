...
export function PublicContentPanel() {
  ...
  const saveTutor = async (tutor: TutorProfile) => {
    ...
    try {
      ...
      setMessage(`Could not save ${tutor.name}. Approved profiles need a biography.`);
    } finally {
      setSaving(null);
    }
  };
  ...
  const saveContent = async (record: PublicContent) => {
    ...
    try {
      ...
      setMessage(`Could not save ${record.title}.`);
    } finally {
      setSaving(null);
    }
  };
        ...
        {tutors.map((tutor) => (
          <Card key={tutor.id}>
            ...
            <CardContent className="space-y-4">
              ...
              <Field label="Headshot URL"><Input value={tutor.photoUrl ?? ""} onChange={(event) => updateTutor(tutor.id, { photoUrl: event.target.value || null })} /></Field>
              ...
              <Field label="LinkedIn URL"><Input value={tutor.linkedinUrl ?? ""} onChange={(event) => updateTutor(tutor.id, { linkedinUrl: event.target.value || null })} /></Field>
        ...
        <Card>
          ...
          <CardContent className="grid gap-5 lg:grid-cols-2">
            <div className="space-y-4">
              <Field label="Page title"><Input value={success.title} onChange={(event) => updateContent(success.slug, { title: event.target.value })} /></Field>
              <Field label="SEO title"><Input value={success.seoTitle ?? ""} onChange={(event) => updateContent(success.slug, { seoTitle: event.target.value })} /></Field>
              <Field label="SEO description"><Textarea rows={3} value={success.seoDescription ?? ""} onChange={(event) => updateContent(success.slug, { seoDescription: event.target.value })} /></Field>
