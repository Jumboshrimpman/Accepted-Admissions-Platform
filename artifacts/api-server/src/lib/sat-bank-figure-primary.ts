...
/** Leftover OCR column titles smashed into the stem. */
...
/** `model is 1` over `10 times` — stacked scale-factor fraction, bar dropped. */
...
/** PDF/OCR wrappers leaked into the student stem (`Start referenced content`). */
...
/** Tilde/comma graph dumps that survive ASCII-graph stripping. */
...
/** Consecutive axis ticks OCR'd into the stem (`22 23 24 25 26`). */
export function looksAxisTickBleed(text: string | null | undefined): boolean {
  ...
  if (!/\b(?:data set|dot plot|dat plot|scatterplot|the graph|the plot)\b/i.test(raw)) {
    return false;
  }
  const match = raw.match(/\b(\d{1,2}(?:\s+\d{1,2}){3,})\b/);
...
/** `P Q R` / `S T U` vertex labels OCR'd out of a missing figure. */
...
/** Smashed exponents/products: `66 = 66 x x`, `2 x 8 40 32 =`, `P t = 1.02 t`. */
...
/** Unreadable A–D algebra: `y x p = 57 +`, `y px = + 57`, `y = 57 px px`. */
...
function isPlausibleDataTable(headers: string[], rows: string[][]): boolean {
  ...
  // `Corn / yes / 15.1` is a data row used as a header — not a recovered table.
...
/** Poll-style `Angel Cruz 483` / `Terry Smith 320` counts already in the stem. */
export function hasInlineNamedTableValues(text: string | null | undefined): boolean {
  const value = stripSatBankFigureComments(text);
  const pairs = value.match(/[A-Z][a-z]+(?:\s+[A-Z][a-z]+)+\s+\d{2,}/g) ?? [];
...
/** Leftover smashed chart titles mean the table was not recovered cleanly. */
...
export function looksSmashedStackedFraction(text: string | null | undefined): boolean {
  ...
  for (let index = 0; index < lines.length - 1; index += 1) {
    ...
    // Proper-fraction OCR (`1/10`), not a prose number like `area of 600`.
...
export function stemCitesVisual(text: string | null | undefined): boolean {
  ...
  if (compact.includes("inthefigure") || compact.includes("thelinegraphed")) return true;
...
export function hasReadableStudentStem(input: {
  ...
  if (looksExtractionMarkerBleed(haystack) || looksExtractionMarkerBleed(prose)) return false;
...
export function looksGarbledExtractText(text: string | null | undefined): boolean {
  ...
  if (ASCII_GRAPH.test(value) || ASCII_GRAPH.test(raw)) return true;
...
export function isStudentReadableChoiceText(text: string | null | undefined): boolean {
  ...
  if (looksSmashedPiChoice(raw) || looksSmashedPiChoice(value)) return false;
  if (looksSpacedDecimalChoice(raw) || looksSpacedDecimalChoice(value)) return false;
  ...
  if (looksGluedMinusSpacing(raw) || looksGluedMinusSpacing(value)) return false;
...
export function shouldUseFigurePrimary(input: FigurePrimaryInput): boolean {
  ...
  return (
    isExplicitFigurePrimary(input) ||
    looksGarbledExtractText(prepareStudentExtractText(input.prompt))
  );
}
