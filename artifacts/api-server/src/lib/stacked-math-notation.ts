import { MICHELLE_TIMEZONE_EMAILS } from "./client-timezone.ts";

export function isMichelleClientEmail(email?: string | null): boolean {
  const normalized = email?.trim().toLowerCase() ?? "";
  return (MICHELLE_TIMEZONE_EMAILS as readonly string[]).includes(normalized);
}

/** Live Xavier↔Michelle SAT copies, including in-session titles that omit her name. */
...
/**
 * Rebuild College Board extract stacks such as `x\n45\n48\n+\n=` into
 * `x + 45 = 48`. Also accepts the HTML-flattened form `x 45 48 + =`.
...
export function repairMichelleAttemptResultItems<
  T extends {
    prompt?: string | null;
    stimulus?: string | null;
    explanation?: string | null;
...
>(items: readonly T[], enabled: boolean): T[] {
  return items.map((item) => repairMichelleQuizQuestionFields(item, enabled));
}
