...
import {
  ...
  CreateTutorSessionQuestionBody,
  CreateTutorSessionQuestionParams,
  CreateTutorSessionQuestionResponse,
...
import {
  ...
  syncSessionPreworkAfterAssignmentUpdate,
} from "../lib/sat-bank-service";
import { createReusableQuizFromBank } from "../lib/tutor-reusable-quiz";
import { ensureAutogeneratedSatPrework } from "../lib/autogenerated-sat-prework";
import { ensureTaitoEnglishSessionPrework } from "../lib/ielts-style-bank";
import { shouldSkipAutogeneratedPreworkAssign } from "../lib/session-prework-dedupe";
import {
  listLiveBeforeSessionHomework,
  withSessionPreworkLock,
} from "../lib/session-prework-live";
import { seedTaitoFallAgendas } from "../lib/seed-taito-fall-agendas";
import {
  SESSION_QUESTION_SHARED_EDIT_MESSAGE,
  sessionQuestionSnapshotValues,
} from "../lib/session-question-copy";
import {
  addBlankMcqToSessionAssignment,
...
async function ensureSatAssessmentSeed(courseId: string): Promise<void> {
  ...
  async function ensureAssignment(
    ...
    if (!assignment) {
      const created = await withSessionPreworkLock(session.id, async () => {
...
async function ensureSeedData(): Promise<string> {
  ...
  const seededSessions = await db
    ...
    .values(
      TAITO_FALL_2026_SESSIONS.map(({ dateKey, subject, tutorName }) => ({
        ...
        hasHomework: subject === "SAT" || subject === "IELTS",
      })),
    )
    .returning();
  await reconcileTaitoSessions(course.id);
  await ensureTaitoEnglishSessionPrework(course.id).catch(() => 0);
  ...
  await seedTaitoFallAgendas({ courseId: course.id }).catch(() => undefined);
  return course.id;
}
  ...
  async (req: AuthedRequest, res): Promise<void> => {
    ...
    const courseFilter =
      courseIds.length > 0
        ...
        : sql`false`;
    const [courses, sessionRows, assignmentRows, libraryAssets, satBankCollections] =
      await Promise.all([
        db.select().from(coursesTable).where(courseFilter).orderBy(asc(coursesTable.title)),
        db
    ...
    const quizzes = (
      await Promise.all(
        assignmentRows.map(async ({ assignment }) => {
          ...
          const onOwnedSession =
            Boolean(assignment.sessionId) &&
            visibleSessionIds.has(assignment.sessionId!);
          return user.role === "administrator" || reusableBank || onOwnedSession
            ? assignment
            : null;
        }),
      )
    ).filter(
      (assignment): assignment is (typeof assignmentRows)[number]["assignment"] =>
        Boolean(assignment),
    );
    const students =
      user.role === "tutor"
        ...
        : await db
            ...
            .where(
              and(
                ...
                eq(courseMembershipsTable.membershipRole, "student"),
                courseIds.length > 0
                  ? inArray(courseMembershipsTable.courseId, courseIds)
                  : sql`false`,
              ),
            )
    ...
    res.json(
      GetTutorCurriculumResponse.parse({
        programs: await Promise.all(courses.map(adminProgramShape)),
        students,
        sessions,
        quizzes: await Promise.all(quizzes.map(adminAssignmentShape)),
        libraryAssets: libraryAssets.map((asset) => libraryAssetResponse(asset)),
        satBankCollections,
      }),
    );
  },
...
router.post(
  ...
  async (req: AuthedRequest, res): Promise<void> => {
    ...
    const user = req.appUser!;
    const tutorUserId =
      user.role === "tutor" ? user.id : (body.data.tutorUserId ?? user.id);
    const clientUserId = body.data.clientUserId ?? null;
    if (user.role === "tutor") {
      if (!clientUserId) {
        res.status(400).json({ error: "A linked student is required." });
        return;
      }
      if (body.data.tutorUserId && body.data.tutorUserId !== user.id) {
      ...
      if (
        ...
        res.status(404).json({ error: "Student is not linked to this tutor." });
        return;
      }
      if (!(await canAccessCourse(user, body.data.courseId, body.data.subject))) {
    ...
    const [course] = await db
      .select({ id: coursesTable.id })
      .from(coursesTable)
      .where(eq(coursesTable.id, body.data.courseId))
      .limit(1);
    if (!course || !(await validateAdminSessionPeople(clientUserId, tutorUserId))) {
      res.status(404).json({ error: "Program or assigned person not found" });
      return;
    }
    ...
    try {
      conflictWith = await adminSessionConflicts(
        ...
        undefined,
        { checkProvider: true, strictProvider: false },
      );
    } catch (error) {
    ...
    if (conflictWith.length > 0) {
      res.status(409).json({
        ...
        error: "This session conflicts with existing scheduling data.",
    ...
    const [created] = await db
      ...
      .values({
        ...
        title,
        status: body.data.status ?? (user.role === "tutor" ? "published" : "draft"),
        durationMinutes: body.data.durationMinutes,
    ...
    await db.insert(auditLogsTable).values({
      ...
      entityType: "session",
      entityId: created!.id,
      metadata: { courseId: created!.courseId, status: created!.status, source: "tutor" },
    });
    try {
      await ensureAutogeneratedSatPrework(created!.id, user.id);
    } catch {
      // Booking/session creation must succeed even if the SAT bank is empty.
    }
    res
      .status(201)
...
router.post(
  "/tutor/assignments/:assignmentId/questions",
  ensureRole(["tutor", "administrator"]),
  async (req: AuthedRequest, res): Promise<void> => {
    const params = CreateTutorSessionQuestionParams.safeParse(req.params);
    const body = CreateTutorSessionQuestionBody.safeParse(req.body ?? {});
    if (!params.success || !body.success) {
      adminMutationError(res, "Invalid session question.");
      return;
    }
    const [assignment] = await db
      .select()
    ...
    if (!assignment.sessionId) {
      res.status(403).json({ error: SESSION_QUESTION_SHARED_EDIT_MESSAGE });
      return;
    }
    const [session] = await db
      .select()
    ...
    try {
      const created = await addBlankMcqToSessionAssignment({
        assignmentId: assignment.id,
        subject: assignment.subject,
      });
      const hasCustomContent =
        body.data.prompt !== undefined ||
        body.data.choices !== undefined ||
        body.data.correctAnswer !== undefined ||
        body.data.explanation !== undefined;
      const question = hasCustomContent
        ? (
            await updateSessionAssignmentQuestionContent({
              assignmentId: assignment.id,
      ...
      const [link] = await db
        ...
        .where(
          and(
            eq(assignmentQuestionsTable.assignmentId, assignment.id),
            eq(assignmentQuestionsTable.questionId, question.id),
          ),
        )
        .limit(1);
      ...
      await db.insert(auditLogsTable).values({
        ...
        metadata: { questionId: question.id, source: "session-copy" },
      });
      res.status(201).json(
      ...
      const status = typeof (error as { status?: number }).status === "number"
        ? (error as { status: number }).status
        : 400;
      const message = error instanceof Error ? error.message : "Could not add the question.";
...
router.post("/booking/sessions", async (req: AuthedRequest, res): Promise<void> => {
  try {
    ...
    try {
      ...
      try {
        await ensureAutogeneratedSatPrework(confirmed.id, req.appUser!.id);
      } catch {
...
router.patch(
  ...
  async (req: AuthedRequest, res): Promise<void> => {
    ...
    if (req.appUser!.role === "tutor") {
      if (!existing.sessionId) {
        res.status(403).json({ error: "Tutors cannot edit shared bank quizzes." });
        return;
      }
      const [ownedSession] = await db
        ...
        .limit(1);
      if (!ownedSession || !(await canAccessSession(req.appUser!, ownedSession))) {
        res.status(404).json({ error: "Assignment not found" });
        return;
      }
    }
    ...
    if (body.data.status !== undefined && updated!.status !== existing.status) {
      await syncSessionPreworkAfterAssignmentUpdate(updated!);
    }
...
router.post(
  ...
  async (req: AuthedRequest, res): Promise<void> => {
    ...
    if (!(await canAccessSession(req.appUser!, session))) {
      res.status(404).json({ error: "Session not found in this program" });
      return;
    }
    if (req.appUser!.role === "tutor") {
      ...
      if (existing.sessionId) {
        ...
        if (
          !sourceSession ||
          !(await canAccessSession(req.appUser!, sourceSession))
        ) {
          res.status(404).json({ error: "Assignment not found" });
          return;
        }
    ...
    const created = await db.transaction(async (tx) => {
      ...
      if (planned.copiedQuestions.length > 0) {
        const sourceIds = planned.copiedQuestions.map((question) => question.questionId);
        const sourceRows = await tx
          .select()
          .from(questionsTable)
          .where(inArray(questionsTable.id, sourceIds));
        const sourceById = new Map(sourceRows.map((row) => [row.id, row]));
        const snapshotQuestions = [];
        for (const question of planned.copiedQuestions) {
        ...
        if (snapshotQuestions.length > 0) {
          await tx.insert(assignmentQuestionsTable).values(snapshotQuestions);
        }
      }
      await tx.insert(auditLogsTable).values({
        actorUserId: req.appUser!.id,
...
router.get("/sessions/:sessionId", async (req: AuthedRequest, res): Promise<void> => {
  ...
  const blocks = await db
    ...
    .orderBy(asc(curriculumBlocksTable.position));
  const listedAssignments = await listAssignmentsForUser(
    req.appUser!,
    session.courseId,
    session.id,
  );
  const assignments = selectStatusHomework(listedAssignments);
...
router.patch(
  ...
  async (req: AuthedRequest, res): Promise<void> => {
    ...
    const hasContent =
      ...
      body.data.explanation !== undefined;
    if (hasContent) {
      if (!record.assignment.sessionId) {
        res.status(403).json({ error: SESSION_QUESTION_SHARED_EDIT_MESSAGE });
        return;
      }
      const [ownedSession] = await db
        .select()
        .from(sessionsTable)
        .where(eq(sessionsTable.id, record.assignment.sessionId))
        .limit(1);
      if (!ownedSession || !(await canAccessSession(req.appUser!, ownedSession))) {
        res.status(404).json({ error: "Assignment question not found" });
        return;
      ...
      try {
        const edited = await updateSessionAssignmentQuestionContent({
        ...
        const [link] = await db
          ...
          .where(
            and(
              eq(assignmentQuestionsTable.assignmentId, record.assignment.id),
              eq(assignmentQuestionsTable.questionId, edited.questionId),
            ),
          )
          .limit(1);
        const [updated] = link &&
        (body.data.position !== undefined || body.data.predictionFirst !== undefined)
          ? await db
              .update(assignmentQuestionsTable)
              .set({
                ...(body.data.position === undefined ? {} : { position: body.data.position }),
                ...(body.data.predictionFirst === undefined
                  ? {}
                  : { predictionFirst: body.data.predictionFirst }),
              })
              .where(eq(assignmentQuestionsTable.id, link.id))
              .returning()
          : [link ?? record.assignmentQuestion];
        await db.insert(auditLogsTable).values({
          actorUserId: req.appUser!.id,
        ...
        const status = typeof (error as { status?: number }).status === "number"
          ? (error as { status: number }).status
          : 400;
        const message = error instanceof Error ? error.message : "Could not update the question.";
        res.status(status >= 400 && status < 600 ? status : 400).json({ error: message });
        return;
      }
    }
