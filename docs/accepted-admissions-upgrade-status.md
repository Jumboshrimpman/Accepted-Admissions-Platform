...
- Stripe: Checkout and signed webhooks are implemented. Configure `STRIPE_WEBHOOK_SECRET` in host secrets (not Replit) before accepting signed events. Set `APP_ORIGIN` to the canonical HTTPS origin. Production webhook URL must be `https://app.acceptedadmissions.org/api/stripe/webhook` (see `docs/stripe-webhook.md`).
