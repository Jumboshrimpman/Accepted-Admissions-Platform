...
- **Glued “blank” OCR:** `haveblank`, `moreblank`, `/[a-z]blank[a-z]/i`, and other word+blank glues in stems or choices never ship (`glued_blank_ocr`). Live PT8 stingray/kelp RW. Do not pad. Normal `fill in the blank` / `blank space` / `______` stays.
...
Bare A–D never ships. Unlabeled choice lists plus letter buttons never ship. Missing or invalid letter keys never default to A — they are `missing_letter_key` and cannot be composed, rematerialized onto a student quiz, or saved from the admin editor.

Dropped: true SPR, empty/missing/garbage choice text, missing/invalid letter keys, missing stems, graph-only letter-key shells, duplicate/orphan figure fragments, stems that cite a graph/table with no figure and no recovered table, irreparable OCR, math OCR that is missing exponents or operators or is a failed fraction/layout dump, unlabeled A–D lists, pipe/backslash graph residue, mangled `( , x y )` stems, incomplete table crops, leaked geometry leftover in a numeric choice, scrambled `= (…) f(x)` / empty `f(x)( )`, exploded vertex-fraction OCR, character-spaced garbage (`T h e g r a p h`), junk choice D module boilerplate (`Module 2`, `GO ON TO THE NEXT PAGE`), exploded contingency / OCR-as-table dumps, smashed `%` / pipe tables (Q15-style), similar-triangle stems with no figure (`Figuresnotdrawntoscale`), scatterplots with axis-tick OCR (`y U12345678910`), smashed slope/choices (`1–3`, `y=-+103x`), stacked `14x = 2 w + 19 7y` / stray `f(x)` (Q77), stripped-radical triangle area with no A–D (Q78), smashed `2 4x` factoring (Q79), mashed `2x2` / `( , x y)` systems (Q80), incomplete A/B-only sets such as `2/29` `2/58` (Q81), stray `? following` / `ax2` parabola stems (Q82), figure-only triangle/graph shells with wiped A–D (Q83–Q85), smashed `16+30=190 x` (Q88), surfboard stems with no A–D (Q91), `^ h` / `y f(x)` / `point0,0 5` stems (Q92), chipmunk graph-only shells (Q94), smashed `12x3 −5x ? 3` (Q96), glued compact poly OCR `2 –4x –7x = –36` (live Q97 after #77), scrambled `h x` / `x2` table stems (Q99), `270(0.1)x` / `WhatThe function` stems (Q100), incomplete A–C library items (Q104), leading `=(x-10)(x+13) f(x)` stems (Q106), exploded vertex-form `metal 9` / `(-7) 3` OCR (Q107), smashed circle squares `2 2 x` (Q109), garbled prism `92 K 2 cm . 47` stems (Q110), `f X -2 2` graph junk (Q112), character-spaced `x x y 6 5 4 ? + +` (Q113), axis-bleed `12345678910` / missing scatterplot (Q114), dot-plot axis labels bled into the stem (`16 17 18 19 20 Diameter`) (Q115), smashed `x 16( + 15) ?` algebra (Q117), exploded two-way-table OCR under a cropped figure (Q119), and any item whose A–D copy is unavailable. Intact slash-fraction items (Q89 `x/4 + 1 = 33`) stay. Readable run-on systems (Q93 `s + 7 = 27 r = 3`) stay and render with equation breaks. Inequality systems (Q98) stay. Juxtaposition `21px` (Q105) stays. Spaced `2 - 4x - 7x = -36` stays; glued `2 –4x –7x = –36` drops. Students never see “Multiple-choice options unavailable” — those rows are unlinked at rematerialize and hidden at serve time.

Admin Curriculum’s Correct-answer dropdown reads the assignment GET letter key (admins already receive `includeKeys`). It must not look up the course question-bank list and fall back to A. Empty Explanation on SAT diagnostic cards means that bank lookup missed; the stored quiz key is still on GET assignment and is what students are scored against.

`rematerialized.updated` counts bank.`linkedQuestionId` rewrites only. A small number (e.g. 4) after a live-audit rematerialize is expected when most remaining rows were already in sync or not the bank’s canonical linked row. `dropUnusableAssignmentQuestions` still audits every linked item. Bank-wide `rescore-usable` `missing_letter_key` counts SPR/numeric keys in the full extract, not residual junk on the short diagnostic.
