...
## Duplicate live homework

If a Taito English session already has a live before-session diagnostic or routine (including an empty published row), seed / `ensureAutogeneratedEnglishPrework` **skips**. It does not insert another copy.

To archive extras already on prod (keep the newest complete assignment), see `docs/session-prework-dedupe.md`. Do not rematerialize the bank or scrape official IELTS.
